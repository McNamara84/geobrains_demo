...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
        ...
        $stmt = $link->prepare("INSERT INTO `resource` (`doi`, `title`, `version`, `year`, `Licence_licence_id`, `Resource_Type_resource_name_id`, `Language_language_id`) VALUES (?, ?, ?, ?, ?, ?, ?)");

        try {
            $stmt->execute([ $doi, $title, $version, $year, $Licence_licence_id, $Resource_Type_resource_name_id, $Language_language_id ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            ...
            header("location: resource-read.php?resource_id=$new_id");
        } else {
            ...
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <a href="resource-index.php" class="btn btn-secondary"><?php translate('Cancel') ?></a>
                    </form>
