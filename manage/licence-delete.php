...
if(isset($_POST["licence_id"]) && !empty($_POST["licence_id"])){
    ...
    $fileColumns = [];
    if (isset($tables_and_columns_names['licence']['columns'])) {
        foreach ($tables_and_columns_names['licence']['columns'] as $columnName => $columnDetails) {
    ...
    if (!empty($fileColumns)) {
        foreach ($fileColumns as $columnName) {
            ...
            if ($stmt = mysqli_prepare($link, $sql)) {
                // Set parameters
                $param_id = trim($_POST["licence_id"]);
    ...
    $sql = "DELETE FROM `licence` WHERE `licence_id` = ?";
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if (!isset($error)){
            ...
            header("location: licence-index.php");
        }
    }
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?licence_id=" . $_GET["licence_id"]; ?>" method="post">
                    <?php print_error_if_exists(@$error); ?>
                        <div class="alert alert-danger fade-in">
                            <input type="hidden" name="licence_id" value="<?php echo trim($_GET["licence_id"]); ?>"/>
