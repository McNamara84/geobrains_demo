...
if(isset($_POST["resource_name_id"]) && !empty($_POST["resource_name_id"])){
    ...
    $fileColumns = [];
    if (isset($tables_and_columns_names['resource_type']['columns'])) {
        foreach ($tables_and_columns_names['resource_type']['columns'] as $columnName => $columnDetails) {
    ...
    if (!empty($fileColumns)) {
        foreach ($fileColumns as $columnName) {
            ...
            if ($stmt = mysqli_prepare($link, $sql)) {
                $param_id = trim($_POST["resource_name_id"]);
    ...
    $sql = "DELETE FROM `resource_type` WHERE `resource_name_id` = ?";
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if (!isset($error)){
            header("location: resource_type-index.php");
        }
    }
    ...
    if(empty($_GET["resource_name_id"])){
        header("location: error.php");
        exit();
    }
}
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?resource_name_id=" . $_GET["resource_name_id"]; ?>" method="post">
                    <?php print_error_if_exists(@$error); ?>
                        <div class="alert alert-danger fade-in">
                            <input type="hidden" name="resource_name_id" value="<?php echo trim($_GET["resource_name_id"]); ?>"/>
