...
$total_pages_sql = "SELECT COUNT(*) FROM `language`";
$result = mysqli_query($link,$total_pages_sql);
$total_rows = mysqli_fetch_array($result)[0];
$total_pages = ceil($total_rows / $no_of_records_per_page);

//Column sorting on column name
$columns = array('language_id', 'code', 'name');
...
if (!empty($_GET['search'])) {
    ...
    if (strpos('`language`.`language_id`, `language`.`code`, `language`.`name`', ',')) {
        $where_statement .= " AND CONCAT_WS (`language`.`language_id`, `language`.`code`, `language`.`name`) LIKE '%$search%'";
    } else {
        $where_statement .= " AND `language`.`language_id`, `language`.`code`, `language`.`name` LIKE '%$search%'";
    }
...
$group_clause = !empty($order) && $order == 'language_id' ? "GROUP BY `language`.`$order`" : '';
...
if ($isCsvExport) {
    ...
    exportAsCSV($data, $db_name, $tables_and_columns_names, 'language', $link, false);
    exit;
}
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        ...
                        <a href="language-create.php" class="btn btn-success float-right"><?php translate('Add New Record') ?></a>
                        <a href="javascript:history.back()" class="btn btn-secondary float-right mr-2"><?php translate('Back') ?></a>
                    </div>

                    <div class="form-row">
                        <form action="language-index.php" method="get">
                        ...
                        if($result) :
                            if(mysqli_num_rows($result) > 0) :
                                ...
                                <table class='table table-bordered table-striped'>
									...
									echo "<th><a href=?search=$search&order=language_id&sort=".$sort_link.">Language ID</a></th>";
									$columnname = "code";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=code&sort=".$sort_link.">Language Code</a></th>";
									$columnname = "name";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=name&sort=".$sort_link.">Language Name</a></th>";
 ?>
                                    ...
                                    <tbody>
                                        <?php while($row = mysqli_fetch_array($result)): ?>
                                            <tr>
                                                <?php echo "<td>" . htmlspecialchars($row['language_id'] ?? "") . "</td>";
																					echo "<td>";
											...
											echo "</td>"."\n\t\t\t\t\t\t\t\t\t\t\t\t"; ?>
                                                <td>
                                                    ...
                                                    if (!empty($column_id)): ?>
                                                        <a id='read-<?php echo $row['language_id']; ?>' href='language-read.php?language_id=<?php echo $row['language_id']; ?>' title='<?php echo addslashes(translate('View Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-info'><i class='far fa-eye'></i></a>
                                                        <a id='update-<?php echo $row['language_id']; ?>' href='language-update.php?language_id=<?php echo $row['language_id']; ?>' title='<?php echo addslashes(translate('Update Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-warning'><i class='far fa-edit'></i></a>
                                                        <a id='delete-<?php echo $row['language_id']; ?>' href='language-delete.php?language_id=<?php echo $row['language_id']; ?>' title='<?php echo addslashes(translate('Delete Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-danger'><i class='far fa-trash-alt'></i></a>
                                                    <?php else: ?>
