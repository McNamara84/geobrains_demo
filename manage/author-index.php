...
$total_pages_sql = "SELECT COUNT(*) FROM `author`";
$result = mysqli_query($link,$total_pages_sql);
$total_rows = mysqli_fetch_array($result)[0];
$total_pages = ceil($total_rows / $no_of_records_per_page);

//Column sorting on column name
$columns = array('author_id', 'lastname', 'firstname', 'orcid', 'affiliation');
...
if (!empty($_GET['search'])) {
    ...
    if (strpos('`author`.`author_id`, `author`.`lastname`, `author`.`firstname`, `author`.`orcid`, `author`.`affiliation`', ',')) {
        $where_statement .= " AND CONCAT_WS (`author`.`author_id`, `author`.`lastname`, `author`.`firstname`, `author`.`orcid`, `author`.`affiliation`) LIKE '%$search%'";
    } else {
        $where_statement .= " AND `author`.`author_id`, `author`.`lastname`, `author`.`firstname`, `author`.`orcid`, `author`.`affiliation` LIKE '%$search%'";
    }
...
$group_clause = !empty($order) && $order == 'author_id' ? "GROUP BY `author`.`$order`" : '';
...
if ($isCsvExport) {
    ...
    exportAsCSV($data, $db_name, $tables_and_columns_names, 'author', $link, false);
    exit;
}
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        ...
                        <a href="author-create.php" class="btn btn-success float-right"><?php translate('Add New Record') ?></a>
                        <a href="javascript:history.back()" class="btn btn-secondary float-right mr-2"><?php translate('Back') ?></a>
                    </div>

                    <div class="form-row">
                        <form action="author-index.php" method="get">
                        ...
                        if($result) :
                            if(mysqli_num_rows($result) > 0) :
                                ...
                                <table class='table table-bordered table-striped'>
									...
									echo "<th><a href=?search=$search&order=author_id&sort=".$sort_link.">Author ID</a></th>";
									$columnname = "lastname";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									...
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=orcid&sort=".$sort_link.">ORCID</a></th>";
									$columnname = "affiliation";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=affiliation&sort=".$sort_link.">Affiliation</a></th>";
 ?>
                                    ...
                                    <tbody>
                                        <?php while($row = mysqli_fetch_array($result)): ?>
                                            <tr>
                                                <?php echo "<td>" . htmlspecialchars($row['author_id'] ?? "") . "</td>";
																					echo "<td>";
											...
											echo nl2br(htmlspecialchars($row['orcid'] ?? ""));
											if ($has_link_file){
											    echo $is_file ? "</a>" : "";
											}
											echo "</td>"."\n\t\t\t\t\t\t\t\t\t\t\t\t";											echo "<td>";
											...
											echo nl2br(htmlspecialchars($row['affiliation'] ?? ""));
											if ($has_link_file){
											    echo $is_file ? "</a>" : "";
											}
											echo "</td>"."\n\t\t\t\t\t\t\t\t\t\t\t\t"; ?>
                                                <td>
                                                    ...
                                                    $column_id = 'author_id';
                                                    if (!empty($column_id)): ?>
                                                        <a id='read-<?php echo $row['author_id']; ?>' href='author-read.php?author_id=<?php echo $row['author_id']; ?>' title='<?php echo addslashes(translate('View Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-info'><i class='far fa-eye'></i></a>
                                                        <a id='update-<?php echo $row['author_id']; ?>' href='author-update.php?author_id=<?php echo $row['author_id']; ?>' title='<?php echo addslashes(translate('Update Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-warning'><i class='far fa-edit'></i></a>
                                                        <a id='delete-<?php echo $row['author_id']; ?>' href='author-delete.php?author_id=<?php echo $row['author_id']; ?>' title='<?php echo addslashes(translate('Delete Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-danger'><i class='far fa-trash-alt'></i></a>
                                                    <?php else: ?>
