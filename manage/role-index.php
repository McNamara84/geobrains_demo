...
$columns = array('role_id', 'name', 'description');
$order = 'role_id';
...
if (!empty($_GET['search'])) {
    ...
    if (strpos('`role`.`role_id`, `role`.`name`, `role`.`description`', ',')) {
        $where_statement .= " AND CONCAT_WS (`role`.`role_id`, `role`.`name`, `role`.`description`) LIKE '%$search%'";
    } else {
        $where_statement .= " AND `role`.`role_id`, `role`.`name`, `role`.`description` LIKE '%$search%'";
    }
...
$group_clause = !empty($order) && $order == 'role_id' ? "GROUP BY `role`.`$order`" : '';
...
if ($isCsvExport) {
    ...
    exportAsCSV($data, $db_name, $tables_and_columns_names, 'role', $link, false);
    exit;
}
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        ...
                        <a href="role-create.php" class="btn btn-success float-right"><?php translate('Add New Record') ?></a>
                        ...
                        if($result) :
                            if(mysqli_num_rows($result) > 0) :
                                ...
                                <table class='table table-bordered table-striped'>
									...
									echo "<th><a href=?search=$search&order=role_id&sort=".$sort_link.">Role ID</a></th>";
									$columnname = "name";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=name&sort=".$sort_link.">Role Name</a></th>";
									$columnname = "description";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=description&sort=".$sort_link.">Role Description</a></th>";
 ?>
                                    ...
                                    <tbody>
                                        <?php while($row = mysqli_fetch_array($result)): ?>
                                            <tr>
                                                <?php echo "<td>" . htmlspecialchars($row['role_id'] ?? "") . "</td>";
																					echo "<td>";
											...
											echo "</td>"."\n\t\t\t\t\t\t\t\t\t\t\t\t"; ?>
                                                <td>
                                                    ...
                                                    $column_id = 'role_id';
                                                    if (!empty($column_id)): ?>
                                                        <a id='read-<?php echo $row['role_id']; ?>' href='role-read.php?role_id=<?php echo $row['role_id']; ?>' title='<?php echo addslashes(translate('View Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-info'><i class='far fa-eye'></i></a>
                                                        <a id='update-<?php echo $row['role_id']; ?>' href='role-update.php?role_id=<?php echo $row['role_id']; ?>' title='<?php echo addslashes(translate('Update Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-warning'><i class='far fa-edit'></i></a>
                                                        <a id='delete-<?php echo $row['role_id']; ?>' href='role-delete.php?role_id=<?php echo $row['role_id']; ?>' title='<?php echo addslashes(translate('Delete Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-danger'><i class='far fa-trash-alt'></i></a>
                                                    <?php else: ?>
