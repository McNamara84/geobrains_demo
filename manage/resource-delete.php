...
if(isset($_POST["resource_id"]) && !empty($_POST["resource_id"])){
    ...
    $fileColumns = [];
    if (isset($tables_and_columns_names['resource']['columns'])) {
        foreach ($tables_and_columns_names['resource']['columns'] as $columnName => $columnDetails) {
    ...
    if (!empty($fileColumns)) {
        foreach ($fileColumns as $columnName) {
            ...
            if ($stmt = mysqli_prepare($link, $sql)) {
                $param_id = trim($_POST["resource_id"]);
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?resource_id=" . $_GET["resource_id"]; ?>" method="post">
                    <?php print_error_if_exists(@$error); ?>
                        <div class="alert alert-danger fade-in">
                            <input type="hidden" name="resource_id" value="<?php echo trim($_GET["resource_id"]); ?>"/>
